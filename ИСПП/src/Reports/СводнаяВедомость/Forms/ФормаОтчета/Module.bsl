...
Процедура Сформировать()
	...
	Если НЕ ЗначениеЗаполнено(Отчет.ПроектЗадания) Тогда
		Сообщить("Не выбран проект задания!");
		Возврат;
	КонецЕсли;	
...
Процедура СформироватьНаСервере()
	...
	ОтчетТабличныйДокумент.АвтоМасштаб = Истина;
	ОтчетТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
	...
	ПараметрыШапки.Вставить("ДатаОтчета", Формат(Отчет.Период.ДатаОкончания,"ДФ=dd.MM.yyyy"));
	ПараметрыШапки.Вставить("НомерЗадания", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отчет.ПроектЗадания,"Номер"), Истина, Истина));	
	
	Область.Параметры.Заполнить(ПараметрыШапки);
	
	ОтчетТабличныйДокумент.Вывести(Область);
	 	
	Область = Макет.ПолучитьОбласть("ШапкаТаблицы");
	
	ОтчетТабличныйДокумент.Вывести(Область);
	
	ДанныеДляОтчета = ПолучитьДанныеДляОтчета();
	
	МассивСерий = Новый Массив;
	
	ВыборкаПоСерии = ДанныеДляОтчета.ВыборкаСерияНоменклатуры;
	
	Пока ВыборкаПоСерии.Следующий() Цикл
		МассивСерий.Добавить(ВыборкаПоСерии.СерияНоменклатуры);	
	КонецЦикла;	
	
	ДанныеДляОтчета.ВыборкаСерияНоменклатуры.Сбросить();
	...
	Пока ВыборкаПоСерии.Следующий() Цикл
		...
		ОбластьСтрокаГруппировки = Макет.ПолучитьОбласть("СтрокаГруппировка");
		
		СтруктураПараметровГруппировки = Новый Структура;
		СтруктураПараметровГруппировки.Вставить("КоличествоПоступление", ВыборкаПоСерии.КоличествоПоступление);
		СтруктураПараметровГруппировки.Вставить("РегНомер", ВыборкаПоСерии.ПредставлениеСерии);
		СтруктураПараметровГруппировки.Вставить("Наименование", ВыборкаПоСерии.ПредставлениеНоменклатуры);
		СтруктураПараметровГруппировки.Вставить("КоличествоПеремещено", ВыборкаПоСерии.КоличествоПеремещено);
		СтруктураПараметровГруппировки.Вставить("КоличествоСписано", ВыборкаПоСерии.КоличествоСписано);
						
		ОбластьСтрокаГруппировки.Параметры.Заполнить(СтруктураПараметровГруппировки);

		МассивВыводимыхОбластей.Добавить(ОбластьСтрокаГруппировки);
		МассивИтогов.Добавить(ОбластьСтрокаГруппировки);
		
		ДанныеПоСтрокам = ВыборкаПоСерии.Выбрать();
		
		НомерСтроки = 1;
		
		Пока ДанныеПоСтрокам.Следующий() Цикл

			ОбластьСтрокаТаблицы = Макет.ПолучитьОбласть("СтрокаТаблицы");
			//ОбластьСтрокаТаблицы.Параметры.Заполнить(ДанныеПоСтрокам);

			Если ДанныеПоСтрокам.ЭтоПоступление Тогда

				ПараметрыСтроки = Новый Структура;
				ПараметрыСтроки.Вставить("НомерПП", НомерСтроки);

				РеквизитыАкта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПоСтрокам.Акт,
					"НомерВходящегоДокумента, ДатаВходящегоДокумента");

				ПараметрыСтроки.Вставить("Расписки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"%1 от %2", РеквизитыАкта.НомерВходящегоДокумента, Формат(РеквизитыАкта.ДатаВходящегоДокумента,
					"ДФ=dd.MM.yyyy;")));
				
				ПараметрыСтроки.Вставить("НомерПоСпецификации", ПолучитьНомерПоСпецификации(
					ДанныеДляОтчета.ТаблицаКодовСтроки, ДанныеПоСтрокам.СпецификацияКДоговору,
					ДанныеПоСтрокам.Номенклатура));

				ПараметрыСтроки.Вставить("РегНомер", ДанныеПоСтрокам.ПредставлениеСерии);
				ПараметрыСтроки.Вставить("Наименование", ДанныеПоСтрокам.ПредставлениеНоменклатуры);
				ПараметрыСтроки.Вставить("Производитель", ДанныеПоСтрокам.ПроизводительНоменклатуры);
				ПараметрыСтроки.Вставить("КоличествоПоступление", ДанныеПоСтрокам.КоличествоПоступление);
				ПараметрыСтроки.Вставить("Акт", ДанныеПоСтрокам.Акт);
			...
			ИначеЕсли ДанныеПоСтрокам.ЭтоПеремещение Тогда

				ПараметрыСтроки = Новый Структура;
				ПараметрыСтроки.Вставить("НомерПП", НомерСтроки);

				Если ТипЗнч(ДанныеПоСтрокам.Акт) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда

					РеквизитыАкта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПоСтрокам.Акт,
						"СкладПолучатель.Наименование, Номер, Дата");

					ПараметрыСтроки.Вставить("ПредприятиеПолучатель", РеквизитыАкта.СкладПолучательНаименование);

				Иначе
					РеквизитыАкта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеПоСтрокам.Акт,
						"Склад.Наименование, Номер, Дата");

					ПараметрыСтроки.Вставить("ПредприятиеПолучатель", РеквизитыАкта.СкладНаименование);
				КонецЕсли;

				ПараметрыСтроки.Вставить("ДокументПередачиПолучателю", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"%1 от %2", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(РеквизитыАкта.Номер, Истина, Истина),
					Формат(РеквизитыАкта.Дата, "ДФ=dd.MM.yyyy;")));

				ПараметрыСтроки.Вставить("РегНомер", ДанныеПоСтрокам.ПредставлениеСерии);
				ПараметрыСтроки.Вставить("Наименование", ДанныеПоСтрокам.ПредставлениеНоменклатуры);

				ПараметрыСтроки.Вставить("КоличествоПеремещено", ДанныеПоСтрокам.КоличествоПеремещено);
				ПараметрыСтроки.Вставить("АктПеремещение", ДанныеПоСтрокам.Акт);
					ПараметрыСтроки.Вставить("НомерПоСпецификации", ПолучитьНомерПоСпецификации(
					ДанныеДляОтчета.ТаблицаКодовСтроки, ДанныеПоСтрокам.СпецификацияКДоговору,
					ДанныеПоСтрокам.Номенклатура));
			...
			ИначеЕсли ДанныеПоСтрокам.ЭтоСписание Тогда

				Если ТипЗнч(ДанныеПоСтрокам.Акт) = Тип("ДокументСсылка.УстановкаУничтожениеМСИ") Тогда

					//@skip-check bsl-legacy-check-string-literal
					ПредприятиеИзготовительКонечногоИзделия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеПоСтрокам.Акт, "Склад.НаименованиеДочернейОрганизации");	
				
					СтруктураПоиска = Новый Структура;
					СтруктураПоиска.Вставить("Акт", ДанныеПоСтрокам.Акт);
					СтруктураПоиска.Вставить("СерияНоменклатуры", ДанныеПоСтрокам.СерияНоменклатуры);
					СтруктураПоиска.Вставить("Номенклатура", ДанныеПоСтрокам.Номенклатура);
					СтруктураПоиска.Вставить("Партия", ДанныеПоСтрокам.Партия);
					
					МассивСтрокУстановок = ДанныеДляОтчета.ТаблицаУстановок.НайтиСтроки(СтруктураПоиска);
					
					Для Каждого СтрокаУстановки ИЗ МассивСтрокУстановок Цикл
						...
						КоличествоСписано = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1; %2; %3",
						Формат(СтрокаУстановки.Количество, "ЧДЦ=2; ЧГ=;"), СтрокаУстановки.Образец, СтрокаУстановки.СерийныйНомерОбразца);
											
						ПараметрыСтроки.Вставить("РегНомер", ДанныеПоСтрокам.ПредставлениеСерии);
						ПараметрыСтроки.Вставить("Наименование", ДанныеПоСтрокам.ПредставлениеНоменклатуры);
						ПараметрыСтроки.Вставить("ПредприятиеИзготовительКонечногоИзделия", ПредприятиеИзготовительКонечногоИзделия);
						ПараметрыСтроки.Вставить("КоличествоСписано", КоличествоСписано);
						ПараметрыСтроки.Вставить("АктУстановки", ДанныеПоСтрокам.Акт);
						ПараметрыСтроки.Вставить("СтоимостьПриУстановкеВИзделие", Формат(СтрокаУстановки.СуммаФактРасхода,"ЧДЦ=2; ЧГ=;"));
...
//			ИначеЕсли ДанныеПоСтрокам.ЭтоУничтожение Тогда
				
//				ОбластьСтрокаТаблицы.Параметры.АктСписания = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.Печать_НомерУведомлениеОСнятииСУчета, ДанныеПоСтрокам.Акт.Печать_ДатаУведомленияОСнятииСУчета);
//				ОбластьСтрокаТаблицы.Параметры.АктСписанияРасшифровка = ДанныеПоСтрокам.Акт;
//				
//				ОбластьСтрокаТаблицы.Параметры.АктУничтожения = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.Печать_НомерАктаУничтожения, ДанныеПоСтрокам.Акт.Печать_ДатаАктаУничтожения);
//				ОбластьСтрокаТаблицы.Параметры.АктУничтоженияРасшифровка = ДанныеПоСтрокам.Акт;

			КонецЕсли;

		КонецЦикла;	
						
		Для Каждого ОбластьДляВывода ИЗ МассивВыводимыхОбластей Цикл
			ОтчетТабличныйДокумент.Вывести(ОбластьДляВывода);	
		КонецЦикла;	
		
	КонецЦикла;
	
	Если Отчет.ВыводитьИтоги Тогда
		
		ОтчетТабличныйДокумент.Вывести(Макет.ПолучитьОбласть("НадписьИтого"));
		ОтчетТабличныйДокумент.Вывести(Макет.ПолучитьОбласть("ШапкаТаблицы"));
		
		Для Каждого ОбластьДляВывода ИЗ МассивИтогов Цикл
			ОтчетТабличныйДокумент.Вывести(ОбластьДляВывода);	
		КонецЦикла;
...
// Получить номер по спецификации.
// 
// Параметры:
//  ТаблицаКодовСтроки - ТаблицаЗначений, ДеревоЗначений - Таблица кодов строки
//  СпецификацияКДоговору - ДокументСсылка.СпецификацияКДоговору -  к договору
//  Номенклатура - СправочникСсылка.Номенклатура - Номенклатура
// 
// Возвращаемое значение:
//  Число
&НаСервере
Функция ПолучитьНомерПоСпецификации(ТаблицаКодовСтроки, СпецификацияКДоговору, Номенклатура)
	
	СтруктураПоиска = Новый Структура;
	СтруктураПоиска.Вставить("СпецификацияКДоговору", СпецификацияКДоговору);
	СтруктураПоиска.Вставить("Номенклатура", Номенклатура);
	
	СтрокиКодов = ТаблицаКодовСтроки.НайтиСтроки(СтруктураПоиска);
	
	Если СтрокиКодов.Количество() Тогда
		Возврат СтрокиКодов[0].КодСтроки;	
	КонецЕсли;	
...
&НаСервере
Функция СформироватьОписаниеДокумента(НомерДокумента, Дата)
...
Функция ПолучитьДанныеДляОтчета()
	...
	//ОтбиратьПроектЗадания = Отчет.ПроектЗадания <> Неопределено И НЕ Отчет.ПроектЗадания.Пустая();
		
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаОтчет();
	...
	Запрос.УстановитьПараметр("ПроектЗадания", Отчет.ПроектЗадания);
	
	СтруктураДанных = Новый Структура;
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	СтруктураДанных.Вставить("ВыборкаСерияНоменклатуры", РезультатЗапроса[2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам));
	СтруктураДанных.Вставить("ТаблицаУстановок", РезультатЗапроса[3].Выгрузить());
	СтруктураДанных.Вставить("ТаблицаКодовСтроки", РезультатЗапроса[4].Выгрузить());
...
Функция ТекстЗапросаОтчет()
	
	Возврат "ВЫБРАТЬ
	        |	ТоварыНаСкладахОбороты.Номенклатура КАК Номенклатура,
	        |	ТоварыНаСкладахОбороты.Организация КАК Организация,
	        ...
	        |	ТоварыНаСкладахОбороты.Партия.ДокументОприходования.ПроектЗадания КАК ПроектЗадания,
	        |	ВЫБОР
	        |		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) В (ТИП(Документ.КомплектацияНоменклатуры), ТИП(Документ.ПоступлениеТоваров), ТИП(Документ.ПрочееОприходованиеТоваров))
	        ...
	        |	ВЫБОР
	        |		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) В (ТИП(Документ.ПеремещениеТоваров), ТИП(Документ.ПередачаТоваровНаСторону))
	        ...
	        |	ВЫБОР
	        |		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.УстановкаУничтожениеМСИ)
	        |				И ТоварыНаСкладахОбороты.Регистратор.ВидАктаСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие)
	        |			ТОГДА ТоварыНаСкладахОбороты.КоличествоРасход
	        |		ИНАЧЕ 0
	        ...
	        |	ВЫБОР
	        |		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.УстановкаУничтожениеМСИ)
	        |				И ТоварыНаСкладахОбороты.Регистратор.ВидАктаСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.Уничтожение)
	        |			ТОГДА ТоварыНаСкладахОбороты.КоличествоРасход
	        |		ИНАЧЕ 0
	        |	КОНЕЦ КАК КоличествоУничтожено,
	        |	ВЫБОР
	        |		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.УстановкаУничтожениеМСИ)
	        |				И ТоварыНаСкладахОбороты.Регистратор.ВидАктаСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.Уничтожение)
	        |			ТОГДА ИСТИНА
	        |		ИНАЧЕ ЛОЖЬ
	        |	КОНЕЦ КАК ЭтоУничтожение,
	        |	ТоварыНаСкладахОбороты.Регистратор КАК Акт,
	        |	ТоварыНаСкладахОбороты.Регистратор.Дата КАК ДатаАкта,
	        |	ТоварыНаСкладахОбороты.Регистратор.Номер КАК НомерАкта
	        ...
	        |					И ТоварыНаСкладахОбороты.Склад = ТоварыНаСкладахОбороты.Регистратор.СкладОтправитель
	        |				ТОГДА ИСТИНА
	        |			КОГДА ТоварыНаСкладахОбороты.Регистратор ССЫЛКА Документ.ПеремещениеТоваров
	        |					И ТоварыНаСкладахОбороты.Склад <> ТоварыНаСкладахОбороты.Регистратор.СкладОтправитель
	        |				ТОГДА ЛОЖЬ
	        |			ИНАЧЕ ИСТИНА
	        |		КОНЕЦ
	        |	И ТоварыНаСкладахОбороты.Партия.ДокументОприходования.ПроектЗадания = &ПроектЗадания
	        |
	        |ИНДЕКСИРОВАТЬ ПО
	        |	Акт
	        |;
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВТ_Подготовка.Акт КАК Акт,
	        |	ЕСТЬNULL(ИсполнениеЗаявокПредприятий.СпецификацияКДоговору, ЗНАЧЕНИЕ(Документ.СпецификацияКДоговору.ПустаяСсылка)) КАК СпецификацияКДоговору
	        |ПОМЕСТИТЬ ВТ_Спецификации
	        |ИЗ
	        |	ВТ_Подготовка КАК ВТ_Подготовка
	        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИсполнениеЗаявокПредприятий КАК ИсполнениеЗаявокПредприятий
	        |		ПО ВТ_Подготовка.Акт = ИсполнениеЗаявокПредприятий.Регистратор
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВТ_Подготовка.Акт,
	        |	ЕСТЬNULL(ИсполнениеЗаявокПредприятий.СпецификацияКДоговору, ЗНАЧЕНИЕ(Документ.СпецификацияКДоговору.ПустаяСсылка))
	        |
	        |ИНДЕКСИРОВАТЬ ПО
	        |	Акт
	        |;
	        ...
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВТ_Подготовка.Номенклатура КАК Номенклатура,
	        |	ВТ_Подготовка.Организация КАК Организация,
	        |	ВТ_Подготовка.Склад КАК Склад,
	        |	ВТ_Подготовка.СерияНоменклатуры КАК СерияНоменклатуры,
	        |	ВТ_Подготовка.Партия КАК Партия,
	        |	ВТ_Подготовка.ПроектЗадания КАК ПроектЗадания,
	        |	ВТ_Подготовка.КоличествоПоступление КАК КоличествоПоступление,
	        |	ВТ_Подготовка.ЭтоПоступление КАК ЭтоПоступление,
	        |	ВТ_Подготовка.КоличествоПеремещено КАК КоличествоПеремещено,
	        |	ВТ_Подготовка.ЭтоПеремещение КАК ЭтоПеремещение,
	        |	ВТ_Подготовка.КоличествоСписано КАК КоличествоСписано,
	        |	ВТ_Подготовка.ЭтоСписание КАК ЭтоСписание,
	        |	ВТ_Подготовка.КоличествоУничтожено КАК КоличествоУничтожено,
	        |	ВТ_Подготовка.ЭтоУничтожение КАК ЭтоУничтожение,
	        |	ВТ_Подготовка.Акт КАК Акт,
	        |	ВТ_Подготовка.ДатаАкта КАК ДатаАкта,
	        |	ВТ_Подготовка.НомерАкта КАК НомерАкта,
	        |	ВТ_Спецификации.СпецификацияКДоговору КАК СпецификацияКДоговору,
	        |	ВТ_Подготовка.СерияНоменклатуры.Представление КАК ПредставлениеСерии,
	        |	ВТ_Подготовка.СерияНоменклатуры.Владелец.Представление КАК ПредставлениеНоменклатуры,
	        |	ВТ_Подготовка.Номенклатура.Производитель.Представление КАК ПроизводительНоменклатуры
	        |ИЗ
	        |	ВТ_Подготовка КАК ВТ_Подготовка
	        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Спецификации КАК ВТ_Спецификации
	        |		ПО ВТ_Подготовка.Акт = ВТ_Спецификации.Акт
	        |ГДЕ
	        |	ВТ_Подготовка.ПроектЗадания = &ПроектЗадания
	        |ИТОГИ
	        |	СУММА(КоличествоПоступление),
	        |	СУММА(КоличествоПеремещено),
	        |	СУММА(КоличествоСписано),
	        |	СУММА(КоличествоУничтожено)
	        |ПО
	        |	СерияНоменклатуры
	        |;
	        |
	        ...
	        |ВЫБРАТЬ
	        |	ВТ_Подготовка.Акт КАК Акт,
	        |	ДетализацияСписанияМСИПоТипам.Количество КАК Количество,
	        |	ДетализацияСписанияМСИПоТипам.СуммаФактРасхода КАК СуммаФактРасхода,
	        |	ДетализацияСписанияМСИПоТипам.Изделие КАК Изделие,
	        |	ДетализацияСписанияМСИПоТипам.Образец КАК Образец,
	        |	ДетализацияСписанияМСИПоТипам.СерийныйНомерОбразца КАК СерийныйНомерОбразца,
	        |	ДетализацияСписанияМСИПоТипам.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
	        |	ДетализацияСписанияМСИПоТипам.АналитикаУчетаНоменклатуры.СерияНоменклатуры КАК СерияНоменклатуры,
	        |	ДетализацияСписанияМСИПоТипам.АналитикаУчетаНоменклатуры.Партия КАК Партия
	        |ИЗ
	        |	ВТ_Подготовка КАК ВТ_Подготовка
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДетализацияСписанияМСИПоТипам КАК ДетализацияСписанияМСИПоТипам
	        |		ПО ВТ_Подготовка.Акт = ДетализацияСписанияМСИПоТипам.Регистратор
	        |			И (ДетализацияСписанияМСИПоТипам.ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие))
	        |ГДЕ
	        |	ТИПЗНАЧЕНИЯ(ВТ_Подготовка.Акт) = ТИП(Документ.УстановкаУничтожениеМСИ)
	        |	И ВТ_Подготовка.ПроектЗадания = &ПроектЗадания
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        |	ВТ_Спецификации.СпецификацияКДоговору КАК СпецификацияКДоговору,
	        |	МИНИМУМ(СпецификацияКДоговоруТовары.КодСтроки) КАК КодСтроки,
	        |	СпецификацияКДоговоруТовары.Номенклатура КАК Номенклатура
	        |ИЗ
	        |	ВТ_Спецификации КАК ВТ_Спецификации
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СпецификацияКДоговору.Товары КАК СпецификацияКДоговоруТовары
	        |		ПО ВТ_Спецификации.СпецификацияКДоговору = СпецификацияКДоговоруТовары.Ссылка
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВТ_Спецификации.СпецификацияКДоговору,
	        |	СпецификацияКДоговоруТовары.Номенклатура";
	
КонецФункции	

&НаСервере
Функция ПолучитьДополнительныеДанные(МассивСерий)
	...
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки(&Период, СерияНоменклатуры В (&МассивСерий)) КАК ТоварыНаСкладахОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТоварыНаСкладахОбороты.Регистратор КАК Акт
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах.Обороты(, , Регистратор, СерияНоменклатуры В (&МассивСерий)) КАК ТоварыНаСкладахОбороты
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) В (ТИП(Документ.КомплектацияНоменклатуры), ТИП(Документ.ПоступлениеТоваров), ТИП(Документ.ПрочееОприходованиеТоваров))
	|
	|УПОРЯДОЧИТЬ ПО
	...
	ДополнительныеДанные.Вставить("ТаблицаОстатков", МассивРезультатов[0].Выгрузить());
	
	ВыборкаАктСерии = МассивРезультатов[1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	ТаблицаПоставокПоСериям = Новый ТаблицаЗначений;
	ТаблицаПоставокПоСериям.Колонки.Добавить("СерияНоменклатуры");
	ТаблицаПоставокПоСериям.Колонки.Добавить("Акт");
	
	Пока ВыборкаАктСерии.Следующий() Цикл
		ВыборкаАкт = ВыборкаАктСерии.Выбрать();
		ВыборкаАкт.Следующий();
		ЗаполнитьЗначенияСвойств(ТаблицаПоставокПоСериям.Добавить(), ВыборкаАкт); 
	КонецЦикла;	
	
	ДополнительныеДанные.Вставить("ТаблицаПоставокПоСериям", ТаблицаПоставокПоСериям);
...
Функция НайтиНастройкуВарианта(Форма, ИдентификаторЭлемента)
	ПоискНастройкиВарианта = Форма.БыстрыйПоискНастроекВарианта.Получить(ИдентификаторЭлемента);
	Если ПоискНастройкиВарианта = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	КорневойУзелКД = Форма.Отчет.КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(ПоискНастройкиВарианта.ИдентификаторУзлаКД);
