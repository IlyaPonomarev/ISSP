...
//	Область.Параметры.ДатаНачала = Формат(Отчет.Период.ДатаНачала, "ДФ='dd.MM.yyyy ""г.""'");
//	Область.Параметры.ДатаОкончания = Формат(Отчет.Период.ДатаОкончания, "ДФ='dd.MM.yyyy ""г.""'");
	
	ОтветственныеЛица = ОтветственныеЛицаСервер.ПолучитьОтветственныеЛицаОрганизации(Отчет.Организация, Отчет.Период.ДатаОкончания);
	
	ПараметрыШапки.Вставить("РуководительФИО"         , ОтветственныеЛица.РуководительНаименование);
	ПараметрыШапки.Вставить("РуководительДолжность"   , ОтветственныеЛица.РуководительДолжность);
	//@skip-check wrong-string-literal-content
	ПараметрыШапки.Вставить("ПредставлениеОрганизации"   , ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Отчет.Организация, "НаименованиеПолное"));
...
//	
//	Пока ДанныеДляОтчета.Следующий() Цикл
//		МассивСерий.Добавить(ДанныеДляОтчета.СерияНоменклатуры);	
//	КонецЦикла;	
//	
//	ДанныеДляОтчета.Сбросить();
//	
//	ДополнительныеДанные = ПолучитьДополнительныеДанные(МассивСерий); 
//	
//	Если Отчет.ВыводитьИтоги Тогда
//		ТаблицаИтогов = Новый ТаблицаЗначений;
//		ТаблицаИтогов.Колонки.Добавить("АктПриемки");
//		ТаблицаИтогов.Колонки.Добавить("СерияНоменклатуры");
//		ТаблицаИтогов.Колонки.Добавить("Номенклатура");
//		ТаблицаИтогов.Колонки.Добавить("КоличествоПоступление");
//		ТаблицаИтогов.Колонки.Добавить("КоличествоПеремещено");
//		ТаблицаИтогов.Колонки.Добавить("КоличествоСписано");
//		ТаблицаИтогов.Колонки.Добавить("КоличествоУничтожено");
//		ТаблицаИтогов.Колонки.Добавить("ТекущийОстаток");
//	КонецЕсли;	
//	
//	Пока ДанныеДляОтчета.Следующий() Цикл
//		
...
//		ОбластьСтрокаГруппировки.Параметры.Заполнить(ДанныеДляОтчета);
//		ОбластьСтрокаГруппировки.Параметры.Номенклатура = ДанныеДляОтчета.СерияНоменклатуры.Владелец;
//		
//		Для Каждого СтрокаОстаток Из ДополнительныеДанные.ТаблицаОстатков Цикл
//			Если СтрокаОстаток.СерияНоменклатуры = ДанныеДляОтчета.СерияНоменклатуры Тогда
//				ОбластьСтрокаГруппировки.Параметры.ТекущийОстаток = СтрокаОстаток.Остаток;
//				Прервать;
//			КонецЕсли;	
//		КонецЦикла;
//		
//		ПервыйДокументПоСерии = Неопределено;
//		
//		Для Каждого СтрокаПоставки Из ДополнительныеДанные.ТаблицаПоставокПоСериям Цикл
//			Если СтрокаПоставки.СерияНоменклатуры = ДанныеДляОтчета.СерияНоменклатуры Тогда
//				ПервыйДокументПоСерии = СтрокаПоставки.Акт;
//				Прервать;
//			КонецЕсли;	
//		КонецЦикла;
//		
//		ОбластьСтрокаГруппировки.Параметры.АктПриемки = СформироватьОписаниеДокумента(ПервыйДокументПоСерии.Номер, ПервыйДокументПоСерии.Дата);	
//		
//		ДанныеПоСтрокам = ДанныеДляОтчета.Выбрать();
//		
...
//			
//			Если ТипЗнч(ДанныеПоСтрокам.ПроектЗадания) = Тип("ДокументСсылка.ПроектЗадания") Тогда
//				ОбластьСтрокаТаблицы.Параметры.РешениеКомиссии = ДанныеПоСтрокам.ПроектЗадания.НомерРазрешения + ", № " + ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПоСтрокам.ПроектЗадания.НомерПроекта);
//				ОбластьСтрокаТаблицы.Параметры.РешениеКомиссииРасшифровка = ДанныеПоСтрокам.ПроектЗадания;
//			ИначеЕсли ТипЗнч(ДанныеПоСтрокам.ПроектЗадания) = Тип("ДокументСсылка.ГЗ") Тогда
//				ОбластьСтрокаТаблицы.Параметры.РешениеКомиссии = ДанныеПоСтрокам.ПроектЗадания.НомерПисьма + ", № " + ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПоСтрокам.ПроектЗадания.НомерПроекта);	 
//				ОбластьСтрокаТаблицы.Параметры.РешениеКомиссииРасшифровка = ДанныеПоСтрокам.ПроектЗадания;
//			КонецЕсли;	 
//			
//			Если ДанныеПоСтрокам.ЭтоПоступление Тогда
//				
//				Если ДанныеПоСтрокам.Акт = ПервыйДокументПоСерии Тогда
//					ОбластьСтрокаТаблицы.Параметры.АктПриемки = СформироватьОписаниеДокумента(ДанныеПоСтрокам.НомерАкта, ДанныеПоСтрокам.ДатаАкта);
//					ОбластьСтрокаТаблицы.Параметры.АктПриемкиРасшифровка = ДанныеПоСтрокам.Акт;
//				Иначе
//					ОбластьСтрокаТаблицы.Параметры.Допоставка = СформироватьОписаниеДокумента(ДанныеПоСтрокам.НомерАкта, ДанныеПоСтрокам.ДатаАкта);
//					ОбластьСтрокаТаблицы.Параметры.ДопоставкаРасшифровка = ДанныеПоСтрокам.Акт;
//				КонецЕсли;
//				
//				ОбластьСтрокаТаблицы.Параметры.АктПриемоПередачи = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.НомерВходящегоДокумента, ДанныеПоСтрокам.Акт.ДатаВходящегоДокумента);
//				
//			ИначеЕсли ДанныеПоСтрокам.ЭтоПеремещение Тогда
//				ОбластьСтрокаТаблицы.Параметры.АктПеремещения = СформироватьОписаниеДокумента(ДанныеПоСтрокам.НомерАкта, ДанныеПоСтрокам.ДатаАкта);
//				ОбластьСтрокаТаблицы.Параметры.АктПеремещенияРасшифровка = ДанныеПоСтрокам.Акт;
//				
//				
//				ОбластьСтрокаТаблицы.Параметры.Адреса  =  УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(ДанныеПоСтрокам.Акт.СкладПолучатель, 
//				ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.АдресСклада"),
//				ДанныеПоСтрокам.ДатаАкта,
//				Истина);
//				
//				ОбластьСтрокаТаблицы.Параметры.АдресаРасшифровка = 	ДанныеПоСтрокам.Акт.СкладПолучатель;											
//				
//			ИначеЕсли ДанныеПоСтрокам.ЭтоСписание Тогда
//				
//				ОбластьСтрокаТаблицы.Параметры.АктСписания = СформироватьОписаниеДокумента(ДанныеПоСтрокам.НомерАкта, ДанныеПоСтрокам.ДатаАкта);
//				ОбластьСтрокаТаблицы.Параметры.АктСписанияРасшифровка = ДанныеПоСтрокам.Акт;
//				
//				Если ТипЗнч(ДанныеПоСтрокам.Акт) = Тип("ДокументСсылка.ПередачаТоваровНаСторону") Тогда
//					
//					СведенияОбОтправителе     = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПоСтрокам.Акт.Контрагент, ДанныеПоСтрокам.ДатаАкта);
//					ОбластьСтрокаТаблицы.Параметры.Адреса = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОтправителе, "ЮридическийАдрес");
//					ОбластьСтрокаТаблицы.Параметры.АдресаРасшифровка = ДанныеПоСтрокам.Акт.Контрагент;
//					ОбластьСтрокаТаблицы.Параметры.ОтправкаЗаключения = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.Печать_НомерАкта, ДанныеПоСтрокам.Акт.Печать_ДатаАкта);
//					
//					ОбластьСтрокаТаблицы.Параметры.АктСписания = СформироватьОписаниеДокумента(ДанныеПоСтрокам.НомерАкта, ДанныеПоСтрокам.ДатаАкта);
//					ОбластьСтрокаТаблицы.Параметры.АктСписанияРасшифровка = ДанныеПоСтрокам.Акт;
//
...
//				Иначе
//					
//					Если ДанныеПоСтрокам.Акт.ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.УстановкаВИзделие") Тогда
//						ОбластьСтрокаТаблицы.Параметры.ОтправкаЗаключения = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.Печать_НомерЗаключенияВП, ДанныеПоСтрокам.Акт.Печать_ДатаЗаключенияВП);
//					Иначе
//						ОбластьСтрокаТаблицы.Параметры.ОтправкаЗаключения = СформироватьОписаниеДокумента(ДанныеПоСтрокам.Акт.Печать_НомерАктаВыбраковки, ДанныеПоСтрокам.Акт.Печать_ДатаАктаВыбраковки);
//					КонецЕсли;
//					
...
//		КонецЦикла;	
//		
//		МассивВыводимыхОбластей.Вставить(0, ОбластьСтрокаГруппировки);
//		
//		Если Отчет.ВыводитьИтоги Тогда
//			ЗаполнитьЗначенияСвойств(ТаблицаИтогов.Добавить(), ОбластьСтрокаГруппировки.Параметры);
//		КонецЕсли;	
//		
...
//			Область.Параметры.Заполнить(СтрокаИтог);
//			ОтчетТабличныйДокумент.Вывести(Область);
//		КонецЦикла;	
//		
//	КонецЕсли;	
...
Функция ПолучитьДанныеДляОтчета()
	...
	ОтбиратьПоСкладу = ЗначениеЗаполнено(Отчет.Склад);
		
	Запрос = Новый Запрос;
	Запрос.Текст =ТекстЗапросаОтчет(ОтбиратьПроектЗадания, Отчет.ВыводитьПЗ, Отчет.ВыводитьГЗ, ОтбиратьПоСкладу);
	...
	ВыборкаСерияНоменклатуры = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Возврат ВыборкаСерияНоменклатуры;
	
КонецФункции 

&НаСервереБезКонтекста
Функция ТекстЗапросаОтчет(ОтбиратьПроектЗадания, ВыводитьПЗ, ВыводитьГЗ, ОтборПоСкладу)
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.ПередачаТоваровНаСторону) 
	|		ИЛИ (ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.УстановкаУничтожениеМСИ) И ТоварыНаСкладахОбороты.Регистратор.ВидАктаСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие))
	|			ТОГДА ТоварыНаСкладахОбороты.КоличествоРасход
	|		ИНАЧЕ 0
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) = ТИП(Документ.УстановкаУничтожениеМСИ) И ТоварыНаСкладахОбороты.Регистратор.ВидАктаСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.Уничтожение)
	|			ТОГДА ТоварыНаСкладахОбороты.КоличествоРасход
	|		ИНАЧЕ 0
	...
	|ИЗ " + ?(ОтбиратьПроектЗадания,
	"РегистрНакопления.ТоварыНаСкладах.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Партия.ДокументОприходования.ПроектЗадания = &ПроектЗадания)",
	...
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ " + ?(ВыводитьПЗ И НЕ ВыводитьГЗ,			
	"И ТоварыНаСкладахОбороты.Партия.ДокументОприходования.ПроектЗадания ССЫЛКА Документ.ПроектЗадания", "") + ?(НЕ ВыводитьПЗ И ВыводитьГЗ,
	"И ТоварыНаСкладахОбороты.Партия.ДокументОприходования.ПроектЗадания ССЫЛКА Документ.ГЗ", "")+ ?(ОтборПоСкладу,
