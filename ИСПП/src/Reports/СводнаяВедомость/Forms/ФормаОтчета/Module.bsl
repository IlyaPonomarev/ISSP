...
Функция ПолучитьДанныеДляОтчета()
	...
	Если ОтбиратьПроектЗадания Тогда
...
Функция ТекстЗапросаОтчет()
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) В (ТИП(Документ.КомплектацияНоменклатуры),
	|			ТИП(Документ.ПоступлениеТоваров), ТИП(Документ.ПрочееОприходованиеТоваров))
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТоварыНаСкладахОбороты.Регистратор) В (ТИП(Документ.ПеремещениеТоваров),
	|			ТИП(Документ.ПередачаТоваровНаСторону))
	...
	|	ЕСТЬNULL(ИсполнениеЗаявокПредприятий.СпецификацияКДоговору,
	...
	|ВЫБРАТЬ
	|	ВТ_Подготовка.Акт,
	|	ДетализацияСписанияМСИПоТипам.Количество,
	|	ДетализацияСписанияМСИПоТипам.СуммаФактРасхода,
	|	ДетализацияСписанияМСИПоТипам.Изделие,
	|	ДетализацияСписанияМСИПоТипам.Образец,
	|	ДетализацияСписанияМСИПоТипам.СерийныйНомерОбразца,
