...
Процедура Сформировать()
	...
	Если Отчет.ПроектЗадания = Неопределено ИЛИ Отчет.ПроектЗадания.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран проект задания/ГЗ!");
		Возврат;
	КонецЕсли;
...
Процедура СформироватьНаСервере()
	...
	ДатаОтчета = ?(Отчет.Период = Дата(1,1,1), КонецДня(ТекущаяДата()), КонецДня(Отчет.Период)); 
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	
	
	Если Отчет.СУничтожением Тогда
		Макет = ОтчетОбъект.ПолучитьМакет("МакетПоЗаданиюСУничтожением");
	Иначе
		Макет = ОтчетОбъект.ПолучитьМакет("МакетПоЗаданию");
	КонецЕсли;
	
	
	Область = Макет.ПолучитьОбласть("ШапкаОтчета");
	
	Область.Параметры.НомерЗадания = Отчет.ПроектЗадания.Номер;	
	Область.Параметры.ПериодОтчета = Формат(ДатаОтчета,"ДФ='dd.MM.yyyy ""г.""'");
	Область.Параметры.ПроектЗадания = Отчет.ПроектЗадания;
	...
	Область.Параметры.НомерЗадания = Отчет.ПроектЗадания.Номер;
	Область.Параметры.НомерРазрешения = ?(ТипЗнч(Отчет.ПроектЗадания) = Тип("ДокументСсылка.ГЗ"),Отчет.ПроектЗадания.НомерПисьма, Отчет.ПроектЗадания.НомерРазрешения);
	Область.Параметры.ПроектЗадания = Отчет.ПроектЗадания;
	
	ОтчетТабличныйДокумент.Вывести(Область);
	
	ТаблицаНоменклатуры = ПолучитьТаблицуНоменклатуры(ДатаОтчета);
	
	Область = Макет.ПолучитьОбласть("СтрокаТаблицы");
	
	КоличествоПоступилоВсего = 0;
	КоличествоНаУчетеВсего = 0;
	КоличествоВходнойКонтрольВсего = 0;
	КоличествоСписаноВсего = 0;
	КоличествоВходнойКонтрольНеПрошлоВсего = 0;
	КоличествоБракВсего = 0;
	
	Если Отчет.СУничтожением Тогда
		КоличествоУничтоженоВсего = 0;
	КонецЕсли;

	
	Для Каждого СтрокаТаблицыНоменклатуры Из ТаблицаНоменклатуры Цикл
		
		Область.Параметры.Заполнить(СтрокаТаблицыНоменклатуры);
		
		КоличествоПоступилоВсего = КоличествоПоступилоВсего + СтрокаТаблицыНоменклатуры.КоличествоПоступило;
		КоличествоНаУчетеВсего = КоличествоНаУчетеВсего + СтрокаТаблицыНоменклатуры.КоличествоНаУчете;
		КоличествоВходнойКонтрольВсего = КоличествоВходнойКонтрольВсего + СтрокаТаблицыНоменклатуры.КоличествоВходнойКонтроль;
		КоличествоСписаноВсего = КоличествоСписаноВсего + СтрокаТаблицыНоменклатуры.КоличествоСписано;
		КоличествоВходнойКонтрольНеПрошлоВсего = КоличествоВходнойКонтрольНеПрошлоВсего + СтрокаТаблицыНоменклатуры.КоличествоВходнойКонтрольНеПрошло;
		КоличествоБракВсего = КоличествоБракВсего + СтрокаТаблицыНоменклатуры.КоличествоБрак;
		
		Если Отчет.СУничтожением Тогда
			КоличествоУничтоженоВсего = КоличествоУничтоженоВсего + СтрокаТаблицыНоменклатуры.КоличествоУничтожено;
		КонецЕсли;
	...
	Область.Параметры.КоличествоНоменклатур = ТаблицаНоменклатуры.ВыгрузитьКолонку("Номенклатура").Количество(); 
	Область.Параметры.КоличествоПоступилоВсего = КоличествоПоступилоВсего;
	Область.Параметры.КоличествоНаУчетеВсего = КоличествоНаУчетеВсего;
	Область.Параметры.КоличествоВходнойКонтрольВсего = КоличествоВходнойКонтрольВсего;
	Область.Параметры.КоличествоСписаноВсего = КоличествоСписаноВсего;
	Область.Параметры.КоличествоВходнойКонтрольНеПрошлоВсего = КоличествоВходнойКонтрольНеПрошлоВсего;
	Область.Параметры.КоличествоБракВсего = КоличествоБракВсего;
	
	Если Отчет.СУничтожением Тогда
		Область.Параметры.КоличествоУничтоженоВсего = КоличествоУничтоженоВсего;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьТаблицуНоменклатуры(ДатаОтчета)
	...
	ТаблицаНоменклатуры.Колонки.Добавить("КудаСписано", ОбщегоНазначения.ОписаниеТипаСтрока(1024));
	ТаблицаНоменклатуры.Колонки.Добавить("ГдеБрак", ОбщегоНазначения.ОписаниеТипаСтрока(1024));
	
	СгенерироватьОписаниеСписанияИзделий(ТаблицаНоменклатуры, ДатаОтчета);
...
&НаСервере
Процедура СгенерироватьОписаниеСписанияИзделий(ТаблицаНоменклатуры, ДатаОтчета)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Аналитика.Номенклатура КАК Номенклатура,
		|	ДетализацияСписанияМСИПоТипамОбороты.Изделие КАК Изделие,
		|	ДетализацияСписанияМСИПоТипамОбороты.Образец КАК Образец,
		|	ДетализацияСписанияМСИПоТипамОбороты.ТипСписания КАК ТипСписания
		|ИЗ
		|	РегистрНакопления.ДетализацияСписанияМСИПоТипам.Обороты(, &КонецПериода,,
		|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
		|	И ТипСписания В (ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие),
		|		ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.ПередачаНаСторону), ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.БракВИзделии))
		|	И (Изделие <> ЗНАЧЕНИЕ(Справочник.Изделия.ПустаяСсылка)
		|	ИЛИ Образец <> ЗНАЧЕНИЕ(Справочник.ОбразцыВВСТ.ПустаяСсылка))) КАК ДетализацияСписанияМСИПоТипамОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
		|		ПО ДетализацияСписанияМСИПоТипамОбороты.Номенклатура = Аналитика.Номенклатура
		|		И ДетализацияСписанияМСИПоТипамОбороты.СерияНоменклатуры = Аналитика.СерияНоменклатуры
		|		И ДетализацияСписанияМСИПоТипамОбороты.Партия = Аналитика.Партия
		|СГРУППИРОВАТЬ ПО
		|	Аналитика.Номенклатура,
		|	ДетализацияСписанияМСИПоТипамОбороты.Изделие,
		|	ДетализацияСписанияМСИПоТипамОбороты.Образец,
		|	ДетализацияСписанияМСИПоТипамОбороты.ТипСписания
		|ИТОГИ
		|ПО
		|	Номенклатура";
	...
	Для Каждого СтрокаНоменклатура Из ТаблицаНоменклатуры Цикл
		...
		Если ВыборкаНоменклатура.НайтиСледующий(Новый Структура("Номенклатура", СтрокаНоменклатура.Номенклатура)) Тогда 
			
			ВыборкаДетальныеЗаписи = ВыборкаНоменклатура.Выбрать();
			
			ЭтоПерваяСтрока = Истина;
			ЭтоПерваяСтрокаБрак = Истина;
			
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Если ВыборкаДетальныеЗаписи.ТипСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.БракВИзделии") Тогда
					СтрокаНоменклатура.ГдеБрак = СтрокаНоменклатура.ГдеБрак + ?(ЭтоПерваяСтрокаБрак,"",", ") + "(" + ВыборкаДетальныеЗаписи.Изделие + "/" + ВыборкаДетальныеЗаписи.Образец + ")";
					ЭтоПерваяСтрокаБрак = Ложь;
				Иначе	
					СтрокаНоменклатура.КудаСписано = СтрокаНоменклатура.КудаСписано + ?(ЭтоПерваяСтрока,"",", ") + "(" + ВыборкаДетальныеЗаписи.Изделие + "/" + ВыборкаДетальныеЗаписи.Образец + ")";
...
Функция ТекстЗапросаОтчет()
	...
	|	ИсполнениеЗаявокПредприятийОбороты.СпецификацияКДоговору.ПроектЗадания КАК ПроектЗадания,
	|	ИсполнениеЗаявокПредприятийОбороты.КоличествоОборот КАК КоличествоПоступило,
	|	0 КАК КоличествоНаУчете,
	|	0 КАК КоличествоВходнойКонтроль,
	|	0 КАК КоличествоСписано,
	|	0 КАК КоличествоВходнойКонтрольНеПрошло,
	|	0 КАК КоличествоБрак,
	|	0 КАК КоличествоУничтожено
	|ПОМЕСТИТЬ ВТ_ДанныеДляОтчета
	|ИЗ
	|	РегистрНакопления.ИсполнениеЗаявокПредприятий.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.Поступление)) КАК
	|		ИсполнениеЗаявокПредприятийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ИсполнениеЗаявокПредприятийОбороты.Номенклатура,
	|	ИсполнениеЗаявокПредприятийОбороты.СпецификацияКДоговору.ПроектЗадания,
	|	0,
	|	ИсполнениеЗаявокПредприятийОбороты.КоличествоОборот,
	|	0,
	|	0,
	|	0,
	|	0,
	...
	|	РегистрНакопления.ИсполнениеЗаявокПредприятий.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.Поступление)
	|	И (СерияНоменклатуры = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	ИЛИ СерияНоменклатуры.ПоставленаНаУчет)) КАК ИсполнениеЗаявокПредприятийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрНакопления.ИсполнениеЗаявокПредприятий.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.ВходнойКонтроль)) КАК
	|		ИсполнениеЗаявокПредприятийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	РегистрНакопления.ИсполнениеЗаявокПредприятий.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.ВходнойКонтрольНеПройден)) КАК
	|		ИсполнениеЗаявокПредприятийОбороты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Аналитика.Номенклатура,
	|	ДетализацияСписанияМСИПоТипамОбороты.СпецификацияКДоговору.ПроектЗадания,
	|	0,
	|	0,
	|	0,
	|	ДетализацияСписанияМСИПоТипамОбороты.КоличествоОборот,
	|	0,
	|	0,
	|	0
	|ИЗ
	|	РегистрНакопления.ДетализацияСписанияМСИПоТипам.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И (ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие)
	|	ИЛИ ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.ПередачаНаСторону))) КАК
	|		ДетализацияСписанияМСИПоТипамОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО ДетализацияСписанияМСИПоТипамОбороты.Номенклатура = Аналитика.Номенклатура
	|		И ДетализацияСписанияМСИПоТипамОбороты.СерияНоменклатуры = Аналитика.СерияНоменклатуры
	|		И ДетализацияСписанияМСИПоТипамОбороты.Партия = Аналитика.Партия
	...
	|	РегистрНакопления.ДетализацияСписанияМСИПоТипам.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.БракВИзделии)) КАК ДетализацияСписанияМСИПоТипамОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО ДетализацияСписанияМСИПоТипамОбороты.Номенклатура = Аналитика.Номенклатура
	|		И ДетализацияСписанияМСИПоТипамОбороты.СерияНоменклатуры = Аналитика.СерияНоменклатуры
	|		И ДетализацияСписанияМСИПоТипамОбороты.Партия = Аналитика.Партия
	...
	|	РегистрНакопления.ДетализацияСписанияМСИПоТипам.Обороты(, &КонецПериода,,
	|		СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	|	И ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.Уничтожение)) КАК ДетализацияСписанияМСИПоТипамОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО ДетализацияСписанияМСИПоТипамОбороты.Номенклатура = Аналитика.Номенклатура
	|		И ДетализацияСписанияМСИПоТипамОбороты.СерияНоменклатуры = Аналитика.СерияНоменклатуры
	|		И ДетализацияСписанияМСИПоТипамОбороты.Партия = Аналитика.Партия
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДанныеДляОтчета.Номенклатура КАК Номенклатура,
	|	ВТ_ДанныеДляОтчета.ПроектЗадания КАК ПроектЗадания,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоПоступило) КАК КоличествоПоступило,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоНаУчете) КАК КоличествоНаУчете,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоВходнойКонтроль) КАК КоличествоВходнойКонтроль,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоСписано) КАК КоличествоСписано,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоВходнойКонтрольНеПрошло) КАК КоличествоВходнойКонтрольНеПрошло,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоБрак) КАК КоличествоБрак,
	|	СУММА(ВТ_ДанныеДляОтчета.КоличествоУничтожено) КАК КоличествоУничтожено
	|ИЗ
	|	ВТ_ДанныеДляОтчета КАК ВТ_ДанныеДляОтчета
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ДанныеДляОтчета.Номенклатура,
	|	ВТ_ДанныеДляОтчета.ПроектЗадания";
