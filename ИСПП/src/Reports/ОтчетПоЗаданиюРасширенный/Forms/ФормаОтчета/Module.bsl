...
Процедура СгенерироватьОписаниеСписанияИзделий(ТаблицаНоменклатуры, ДатаОтчета)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДетализацияСписанияМСИПоТипамОбороты.Номенклатура КАК Номенклатура,
		...
		|ИЗ
		|	РегистрНакопления.ДетализацияСписанияМСИПоТипам.Обороты(
		|			,
		|			&КонецПериода,
		|			,
		|			СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
		|				И ТипСписания В (ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.УстановкаВИзделие), ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.ПередачаНаСторону), ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.БракВИзделии))
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ДетализацияСписанияМСИПоТипамОбороты.Номенклатура,
...
Функция ТекстЗапросаОтчет()
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |			,
	        |			СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	        |				И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.Поступление)) КАК ИсполнениеЗаявокПредприятийОбороты
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ
	        ...
	        |			,
	        |			СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	        |				И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.ВходнойКонтроль)) КАК ИсполнениеЗаявокПредприятийОбороты
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ
	        ...
	        |			,
	        |			СпецификацияКДоговору.ПроектЗадания = &ПроектЗадания
	        |				И ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипОперацииИсполненияЗаявкиПредприятия.ВходнойКонтрольНеПройден)) КАК ИсполнениеЗаявокПредприятийОбороты
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ
	        ...
	        |					ИЛИ ТипСписания = ЗНАЧЕНИЕ(Перечисление.ВидыАктовСписаний.ПередачаНаСторону))) КАК ДетализацияСписанияМСИПоТипамОбороты
	        |
	        |ОБЪЕДИНИТЬ ВСЕ
	        |
	        |ВЫБРАТЬ
