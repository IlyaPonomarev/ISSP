...
Процедура УстановитьДоступностьИзмененияСоставаСтрока()
	Элементы.Товары.ИзменятьПорядокСтрок = Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыСпецификацийКДоговорам.НеСогласован");
КонецПроцедуры	
...
Процедура ПроектЗаданияПриИзмененииНаСервере()
	Объект.Валюта = Объект.ПроектЗадания.Валюта;
КонецПроцедуры
...
&НаКлиенте
Процедура СвязанныеЗаявкиПредприятийВыбратьПриИзменении(Элемент)
КонецПроцедуры
