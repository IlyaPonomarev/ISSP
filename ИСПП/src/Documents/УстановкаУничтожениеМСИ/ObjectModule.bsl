...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.БракМСИ") Тогда
		ЗаполнитьДокументНаОснованииБракМСИ(ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.УстановкаВИзделие") Тогда
		НепроверяемыеРеквизиты.Добавить("СпособУничтожения");
	КонецЕсли;	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.Уничтожение") Тогда
		...
		Для Каждого СтрокаТовар Из Товары Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТовар.Изделие) ТОгда
				СтрокаТовар.Изделие = Справочники.Изделия.ПустаяСсылка();
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаТовар.Образец) Тогда
	...
	Иначе
		СогласованАктОтбора = Ложь;
		
		Если ЗначениеЗаполнено(СпособУничтожения) Тогда
			СпособУничтожения = Справочники.СпособыУничтоженияОбразцов.ПустаяСсылка();	
		КонецЕсли;
		
	КонецЕсли;
	
	ДополнительныеСвойства.Вставить("ВидАктаСписания", ВидАктаСписания);
	ДополнительныеСвойства.Вставить("СогласованАктОтбора", СогласованАктОтбора);
...
Процедура ЗаполнитьДокументНаОснованииПеремещенияТоваров(ДокументОснование)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Товары.СуммаФактическихРасходов КАК СуммаФактРасхода
	               |ИЗ
...
Процедура ЗаполнитьПоВходномуКонтролю(ДанныеЗаполнения)
	...
	ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.Уничтожение");
...
Процедура ЗаполнитьДокументНаОснованииБракМСИ(ДанныеЗаполнения)
	...
	ПроектЗадания = ДанныеЗаполнения.ПроектЗадания;
	Гриф = ДанныеЗаполнения.Гриф;
	Центр = ДанныеЗаполнения.Центр;
	АналитикаРасходов = ДанныеЗаполнения.АналитикаРасходов;
	
	ПодразделениеОрганизации = ДанныеЗаполнения.ПодразделениеОрганизации;
	
	Печать_НомерЗаключенияВП = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеЗаполнения.Номер, Истина, Истина);	
	Печать_ДатаЗаключенияВП = ДанныеЗаполнения.Дата;
