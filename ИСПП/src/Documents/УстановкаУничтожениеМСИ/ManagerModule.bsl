...
Процедура СформироватьТаблицыДвиженийДляПроведения(ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	Если ДополнительныеСвойства.ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.УстановкаВИзделие") 
		ИЛИ (ДополнительныеСвойства.ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.Уничтожение") И ДополнительныеСвойства.СогласованАктОтбора) Тогда
		...
		Если ДополнительныеСвойства.ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.Уничтожение") Тогда
...
Функция ТекстЗапросаВтТаблицаТовары()
	...
	|	ВтТаблицаТовары
	|ИЗ
	|	Документ.УстановкаУничтожениеМСИ.Товары КАК ТаблицаТовары
	|ГДЕ
...
Функция ТекстЗапросаТаблицаСписано()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	Документ.УстановкаУничтожениеМСИ.Товары КАК СписаниеНедостачТоваровТовары
	               |ГДЕ
...
Функция ТекстЗапросаТаблицаДетализацииСписанияПоТипам()
	...
	|	СписаниеНедостачТоваровТовары.СерияНоменклатуры КАК СерияНоменклатуры,
	|	СписаниеНедостачТоваровТовары.Партия КАК Партия,
	|	АналитикаУчетаНоменклатуры.КлючАналитики КАК АналитикаУчетаНоменклатуры,
	|	СписаниеНедостачТоваровТовары.ID_Госконтракт КАК Госконтракт,
	|	СписаниеНедостачТоваровТовары.СерийныйНомерОбразца КАК СерийныйНомерОбразца,
	|	СУММА(СписаниеНедостачТоваровТовары.СуммаФактРасхода) КАК СуммаФактРасхода
	|ИЗ
	|	Документ.УстановкаУничтожениеМСИ.Товары КАК СписаниеНедостачТоваровТовары
	...
	|СГРУППИРОВАТЬ ПО
	|	СписаниеНедостачТоваровТовары.Партия.ДокументОприходования.ЗаказПоставщику,
	|	СписаниеНедостачТоваровТовары.Ссылка.Склад,
	|	СписаниеНедостачТоваровТовары.Номенклатура,
	|	СписаниеНедостачТоваровТовары.Ссылка.ВидАктаСписания,
	|	СписаниеНедостачТоваровТовары.Изделие,
	|	СписаниеНедостачТоваровТовары.Образец,
	|	СписаниеНедостачТоваровТовары.СерияНоменклатуры,
	|	СписаниеНедостачТоваровТовары.Партия,
	|	АналитикаУчетаНоменклатуры.КлючАналитики,
	|	СписаниеНедостачТоваровТовары.ID_Госконтракт,
	|	СписаниеНедостачТоваровТовары.СерийныйНомерОбразца";
...
Функция ДоступныеПечатныеФормы() Экспорт
	...
	ПечатнаяФорма.Представление = МетаданныеМакетов.ПФ_MXL_АктСписания.Представление();
	...
	ПечатнаяФорма.Представление = МетаданныеМакетов.ПФ_MXL_ОписьКАктуСписания.Представление();
	...
	ПечатнаяФорма = УправлениеПечатьюПоддержкаПроектов.ДобавитьПечатнуюФорму(ПечатныеФормы, "АктОтбораОбразцов", МенеджерПечати);
	ПечатнаяФорма.Представление = НСтр("ru='Акт отбора образцов'");
	ПечатнаяФорма.ПутьКМакету = ФормированиеПечатныхФормПоддержкаПроектов.ПутьКМакету(МетаданныеМакетов.ПФ_MXL_АктОтбораОбразцов);
	УправлениеПечатьюПоддержкаПроектов.ДобавитьКомандуПечати(ПечатнаяФорма);
	
		ПечатнаяФорма = УправлениеПечатьюПоддержкаПроектов.ДобавитьПечатнуюФорму(ПечатныеФормы, "АктУничтоженияОбразцов", МенеджерПечати);
	ПечатнаяФорма.Представление = НСтр("ru='Акт уничтожения образцов'");
	ПечатнаяФорма.ПутьКМакету = ФормированиеПечатныхФормПоддержкаПроектов.ПутьКМакету(МетаданныеМакетов.ПФ_MXL_АктУничтоженияОбразцов);
	УправлениеПечатьюПоддержкаПроектов.ДобавитьКомандуПечати(ПечатнаяФорма);
...
Функция ПечатьАктСписанияТоваров(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.УстановкаУничтожениеМСИ.ПФ_MXL_АктСписанияТоваров");
...
Функция ПолучитьПараметрыШапкиАктСписания(Шапка, ВыборкаСоставКомиссии)
	...
	Параметры.Вставить("НомерАктаВП", Шапка.НомерАктаУничтожения);
	Параметры.Вставить("ДатаАктаВП", Формат(Шапка.ДатаАктаУничтожения,"ДФ='dd.MM.yyyy ""г.""'"));
	...
	Иначе
		Параметры.Вставить("НомерПриложенияВП", Шапка.НомерАктаУничтожения);
		Параметры.Вставить("ДатаПриложенияВП", Формат(Шапка.ДатаАктаУничтожения,"ДФ=dd.MM.yyyy"));
	КонецЕсли;
...
#Область ПечатьАктОтбораОбразцов

Функция ПечатьАктОтбораОбразцов(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабличныйДокумент.АвтоМасштаб        = Истина;
	
	ПолноеИмяМакета = ФормированиеПечатныхФормПоддержкаПроектов.ПутьКМакету(ПустаяСсылка().Метаданные().Макеты.ПФ_MXL_АктОтбораОбразцов);
	...
	Служебный_ПечатьАктаОтбораОбразцов(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент, ПолноеИмяМакета, Шапка, ВыборкаПоДокументам, КоличествоЭкземпляров, ВыборкаСоставКомиссии);
		
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура Служебный_ПечатьАктаОтбораОбразцов(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент, ПолноеИмяМакета, Шапка, ВыборкаПоДокументам, КоличествоЭкземпляров, ВыборкаСоставКомиссии)
	...
	Пока Шапка.Следующий() Цикл
		...
		Для Экз = 1 По КоличествоЭкземпляров Цикл
			...
			// Получение параметров для заполнения
			ПараметрыИзШапки = ПолучитьПараметрыШапкиАктОтбораОбразцов(Шапка, ВыборкаСоставКомиссии);
			ПараметрыИзШапки.Вставить("Экземпляр", Экз);
			...
			Если ВыборкаПоДокументам.НайтиСледующий(ПараметрыПоиска) Тогда
				...
				Пока ВыборкаСтрокТовары.Следующий() Цикл
					...
					НомерСтроки = НомерСтроки + 1;
					Количество = Количество + ВыборкаСтрокТовары.Количество;
					ДанныеСтроки = Новый Структура("НомерПП, Номенклатура, Производитель, Серия, Количество, Основание, КодГода", НомерСтроки, ВыборкаСтрокТовары.Номенклатура, ВыборкаСтрокТовары.Номенклатура.Производитель, ВыборкаСтрокТовары.СерияНоменклатуры, ВыборкаСтрокТовары.Количество, ВыборкаСтрокТовары.Основание, ВыборкаСтрокТовары.КодГода);
					ОбластьСтрока = Макет.ПолучитьОбласть("Строка");
					ОбластьСтрока.Параметры.Заполнить(ДанныеСтроки);
...
Функция ПолучитьПараметрыШапкиАктОтбораОбразцов(Шапка, ВыборкаСоставКомиссии)
	...
	Параметры.Вставить("РуководительФИО"         , ОтветственныеЛица.РуководительНаименование);
	Параметры.Вставить("РуководительДолжность"   , ОтветственныеЛица.РуководительДолжность);
	Параметры.Вставить("НомерАктаВП", Шапка.НомерАктаВП);
	Параметры.Вставить("ДатаАктаВП", Формат(Шапка.ДатаАктаВП,"ДФ=dd.MM.yyyy"));
...
#КонецОбласти // ПечатьОтбораОбразцов

#Область ПечатьАктУничтоженияОбразцов

Функция ПечатьАктУничтоженияОбразцов(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	ТабличныйДокумент.АвтоМасштаб        = Истина;
	
	ПолноеИмяМакетаАктОтбора = ФормированиеПечатныхФормПоддержкаПроектов.ПутьКМакету(ПустаяСсылка().Метаданные().Макеты.ПФ_MXL_АктОтбораОбразцов);
	
	ПолноеИмяМакетаАктУничтожения = ФормированиеПечатныхФормПоддержкаПроектов.ПутьКМакету(ПустаяСсылка().Метаданные().Макеты.ПФ_MXL_АктУничтоженияОбразцов);
	
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_" + ПолноеИмяМакетаАктУничтожения;

	ПараметрыПечати = Новый Структура;
	ПараметрыПечати.Вставить("ПолучатьЦены");
	
	КоличествоЭкземпляровАктОтбора = 1;
	КоличествоЭкземпляровАктУничтожения = 1;
	...
	//Служебный_ПечатьАктаОтбораОбразцов(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент, ПолноеИмяМакетаАктОтбора, Шапка, ВыборкаПоДокументам, КоличествоЭкземпляровАктОтбора, ВыборкаСоставКомиссии); //Отключено
	
	Служебный_ПечатьАктУничтожениеОбразцов(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент, ПолноеИмяМакетаАктУничтожения, Шапка, ВыборкаПоДокументам, КоличествоЭкземпляровАктУничтожения, ВыборкаСоставКомиссии);	
	
	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура Служебный_ПечатьАктУничтожениеОбразцов(МассивОбъектов, ОбъектыПечати, ТабличныйДокумент, ПолноеИмяМакета, Шапка, ВыборкаПоДокументам, КоличествоЭкземпляров, ВыборкаСоставКомиссии)
	...
	Пока Шапка.Следующий() Цикл
		...
		Для Экз = 1 По КоличествоЭкземпляров Цикл
			...
			// Получение параметров для заполнения
			ПараметрыИзШапки = ПолучитьПараметрыШапкиАктУничтоженияОбразцов(Шапка, ВыборкаСоставКомиссии);
			ПараметрыИзШапки.Вставить("Экземпляр", Экз);
			...
			Если ВыборкаПоДокументам.НайтиСледующий(ПараметрыПоиска) Тогда
				...
				МассивИдентификацииСерий = Новый Массив;
				...
				Пока ВыборкаСтрокТовары.Следующий() Цикл
					...
					Если МассивИдентификацииСерий.Найти(ВыборкаСтрокТовары.СерияНоменклатуры) = Неопределено Тогда
						МассивИдентификацииСерий.Добавить(ВыборкаСтрокТовары.СерияНоменклатуры);
						НомераСерий = НомераСерий + ВыборкаСтрокТовары.СерияНоменклатуры.Наименование + Разделитель;   
					КонецЕсли;	
				...
				ПараметрыИзШапки.Вставить("НомераСерий", Лев(НомераСерий, СтрДлина(НомераСерий) - 1));
				
			КонецЕсли;
			
			ПараметрыИзШапки.Вставить("ВсегоЧислом", Количество);
			ПараметрыИзШапки.Вставить("ВсегоПрописью", ЧислоПрописью(Количество,"НП=Ложь",",,,,,,,,0"));
...
Функция ПолучитьПараметрыШапкиАктУничтоженияОбразцов(Шапка, ВыборкаСоставКомиссии)
	...
	Параметры.Вставить("НомерАктаВП", Шапка.НомерАктаУничтожения);
	Параметры.Вставить("ДатаАктаВП", Формат(Шапка.ДатаАктаУничтожения,"ДФ='dd.MM.yyyy ""г.""'"));
	Параметры.Вставить("СпособУничтожения", Шапка.СпособУничтожения);
...
#КонецОбласти // ПечатьАктУничтоженияОбразцов
...
ТекстЗапроса = "ВЫБРАТЬ
               ...
               |	Документ.Печать_НомерАктаУничтожения КАК НомерАктаУничтожения,
               |	Документ.Печать_ДатаАктаУничтожения КАК ДатаАктаУничтожения,
               |	Документ.СпособУничтожения.ПредставлениеДляПечати КАК СпособУничтожения
               |ПОМЕСТИТЬ втШапка
               |ИЗ
               |	Документ.УстановкаУничтожениеМСИ КАК Документ
               |ГДЕ
               ...
               |	Товары.СерияНоменклатуры.КодГода КАК КодГода
               |ПОМЕСТИТЬ втТовары
               |ИЗ
               |	Документ.УстановкаУничтожениеМСИ.Товары КАК Товары
