...
Процедура ПриОткрытии(Отказ)
	...
	ОбработатьОтображениеЭлементовВЗависимостиОтВидаАктаСписания();
...
&НаКлиенте
Процедура ОбработатьОтображениеЭлементовВЗависимостиОтВидаАктаСписания()
	
	ЭтоУстановка = Объект.ВидАктаСписания = ПредопределенноеЗначение("Перечисление.ВидыАктовСписаний.УстановкаВИзделие");
	
	Элементы.ТоварыИзделие.Видимость = ЭтоУстановка;
	Элементы.ТоварыОбразец.Видимость = ЭтоУстановка;
	Элементы.ТоварыID_Госконтракт.Видимость = ЭтоУстановка;
	Элементы.ТоварыСерийныйНомерОбразца.Видимость = ЭтоУстановка;
	
	Элементы.ТоварыЗаполнитьПоБраку.Видимость = НЕ ЭтоУстановка;
	Элементы.СогласованАктОтбора.Видимость = НЕ ЭтоУстановка;
	Элементы.СпособУничтожения.Видимость = НЕ ЭтоУстановка;
	Элементы.ГруппаУничтожение.Видимость = НЕ ЭтоУстановка;
	
	Если ЭтоУстановка Тогда
		Элементы.Печать_НомерЗаключенияВП.Заголовок = "Номер заключения ВП";
		Элементы.Печать_ДатаЗаключенияВП.Заголовок = "Дата заключения ВП";
	Иначе
		Элементы.Печать_НомерЗаключенияВП.Заголовок = "Номер выбраковки";
		Элементы.Печать_ДатаЗаключенияВП.Заголовок = "Дата акта выбраковки";
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьПоБракуНаСервере()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	БракНаСкладахОстатки.Номенклатура КАК Номенклатура,
		|	БракНаСкладахОстатки.СерияНоменклатуры КАК СерияНоменклатуры,
		|	БракНаСкладахОстатки.Партия КАК Партия,
		|	БракНаСкладахОстатки.КоличествоОстаток КАК Количество
		|ИЗ
		|	РегистрНакопления.БракНаСкладах.Остатки(
		|			&Период,
		|			Организация = &Организация
		|				И Склад = &Склад) КАК БракНаСкладахОстатки";
...
&НаКлиенте
Процедура ЗаполнитьПоБраку(Команда)
	
	Если НЕ ЗначениеЗаполнено(Объект.Организация) И НЕ ЗначениеЗаполнено(Объект.Склад) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не заполнена Организация и Склад!");
		Возврат;
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Склад) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не заполнен Склад!");
		Возврат;
	КонецЕсли;
	
	ОписаниеОповещенияВыбора = Новый ОписаниеОповещения("ЗаполнитьПоБракуЗавершение", ЭтотОбъект);
	
	ПоказатьВопрос(ОписаниеОповещенияВыбора, "Табличная часть будет перезаполнена! Вы уверены, что хотите продолжить?", РежимДиалогаВопрос.ДаНет, 60);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоБракуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		ЗаполнитьПоБракуНаСервере();
		...
		Для Каждого СтрокаТовар Из Объект.Товары Цикл	
			...
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить(Действия.Действие_ПроверитьСериюНоменклатурыПоВладельцу(), СтрокаТовар.СерияНоменклатуры);
			СтруктураДействий.Вставить(Действия.Действие_ПроверитьУпаковкуПоВладельцу(), СтрокаТовар.ЕдиницаИзмерения);
			СтруктураДействий.Вставить(Действия.Действие_ПроверитьПартиюПоВладельцу(), СтрокаТовар.Партия);
			СтруктураДействий.Вставить(Действия.Действие_ЗаполнитьЕдиницуИзмерения());
			СтруктураДействий.Вставить(Действия.Действие_ЗаполнитьПараметрыУчета(), ПараметрыУчетаНоменклатуры);
			СтруктураДействий.Вставить(Действия.Действие_ПересчитатьКоэффициент());
			СтруктураДействий.Вставить(Действия.Действие_ПересчитатьКоличествоЕдиниц());
			
			ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТабличнойЧасти(СтрокаТовар, СтруктураДействий, КэшированныеЗначения);
