...
Функция ТекстЗапросаТаблицаПеремещено()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	СУММА(ПеремещениеТоваровТовары.СуммаФактическихРасходов) КАК СуммаФактРасхода,
	               |	СУММА(ПеремещениеТоваровТовары.СуммаНДС) КАК СуммаНДС,
	               |	СУММА(ПеремещениеТоваровТовары.СуммаСНДС) КАК СуммаСНДС
	               |ИЗ
	               ...
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПеремещениеТоваровТовары.Партия.ДокументОприходования.ЗаказПоставщику,
...
Функция ПолучитьПараметрыШапкиАктПеремещения(Шапка)
	...
	ОтветственныеЛица = ОтветственныеЛицаСервер.ПолучитьОтветственныеЛицаОрганизации(Шапка.Организация,
		Шапка.ДатаДокумента);
	МОЛСдал = РегистрыСведений.МатериальноОтветственныеЛица.ПолучитьДанныеОтветственного(Шапка.СкладОтправитель,
		Шапка.ДатаДокумента);
	МОЛПринял= РегистрыСведений.МатериальноОтветственныеЛица.ПолучитьДанныеОтветственного(Шапка.СкладПолучатель,
		Шапка.ДатаДокумента);
	...
	Параметры.Вставить("РуководительФИО", ФизическиеЛицаКлиентСервер.ФамилияИнициалы(
		ОтветственныеЛица.РуководительНаименование));
	...
	Параметры.Вставить("РуководительДолжностьПолучатель", МОЛПринял.Должность);
	
	Параметры.Вставить("ПолучательРуководительФИО", ?(МОЛПринял.Ответственный.Пустая(), "",
	...
	Параметры.Вставить("СкладПолучательОтветственный", 
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Шапка.Коонтрагент,"ФИООтветственного"));
...
Функция ТекстЗапросаДанныеДляПечати(ПолучатьЦены = Ложь, ПолучатьЦеныПоНастройкамСклада = Ложь)
	...
	|	Документ.СкладПолучатель.НаименованиеДочернейОрганизации КАК ПолучательНаименованиеПолное
	|ПОМЕСТИТЬ втШапка
	|ИЗ
