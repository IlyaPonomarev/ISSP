...
Функция ПолучитьПараметрыШапкиАктПеремещения(Шапка)
	...
	Параметры.Вставить("ПолучательРуководительФИО", ?(Шапка.ПолучательФИООтветственного = Неопределено, "",
		ФизическиеЛицаКлиентСервер.ФамилияИнициалы(Шапка.ПолучательФИООтветственного)));
...
Функция ТекстЗапросаДанныеДляПечати(ПолучатьЦены = Ложь, ПолучатьЦеныПоНастройкамСклада = Ложь)
	...
	|	ЕСТЬNULL(Документ.СкладПолучатель.ДО.ФИООтветственного, """") КАК ПолучательФИООтветственного,
