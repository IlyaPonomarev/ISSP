...
Процедура ТоварыСерияНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УстановитьСерию", ЭтотОбъект);
	
	ОткрытьФорму("Справочник.СерииНоменклатуры.ФормаВыбора", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс); 
КонецПроцедуры
...
Процедура ЗаполнитьЦену(Команда)
	ОписаниеОповещения = Новый ОписаниеОповещения("УстановитьЦену", ЭтотОбъект);
	Число = 0;
...
Процедура ЗаполнитьНомера(Команда)
	...
	ОткрытьФорму("Документ.ПоступлениеТоваров.Форма.ВвестиДанныеПоНомерам", ПараметрыФормы, ЭтотОбъект,,,,ОписаниеОповещения);
КонецПроцедуры
...
Процедура СоздатьСерии(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьСерииЗавершение", ЭтотОбъект);
	
	ПоказатьВопрос(ОписаниеОповещения, "Формировать коды в серийном номере?", РежимДиалогаВопрос.ДаНет);
...
Процедура СоздатьСерииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		ФормироватьКоды = Истина;
	Иначе
		ФормироватьКоды = Ложь;
	КонецЕсли;	
	
	НайтиСоздатьСерииНоменклатурыДоПроведения(ЕстьИзменения, ФормироватьКоды);
...
Процедура ОбработатьИзменениеДоговораКонтрагента()
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		...
		Объект.ПроектЗадания = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДоговорКонтрагента, "ПроектЗадания");
	КонецЕсли;
...
&НаСервере
Процедура НайтиСоздатьСерииНоменклатурыДоПроведения(ЕстьИзменения, ЗаполнятьКоды = Истина)
	...
	Если Объект.Ссылка.Пустая() Тогда
		Для Каждого Строка Из СтрокиССериями Цикл
			...
			Если ЗаполнятьКоды Тогда
				Серия.КодГода = Прав(Формат(Год(Объект.Дата), "ЧГ"), 2);
				Серия.КодЦентра = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Центр, "Код");
				Серия.КодДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПроектЗадания,
					"ДобывающаяОрганизация.Код");
				Серия.КодГрифа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Гриф, "Код");
			КонецЕсли;
	...
	Иначе
		...
		Для Каждого Строка Из Таблица Цикл
			...
			Если Выборка.Следующий() Тогда
				...
				Если Выборка.КоличествоСерий < Строка.Количество И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					Строка.Номенклатура, "ВидНоменклатуры.НастройкаИспользованияСерий")
					...
					Для Каждого Строка Из СтрокиПоНоменклатуре Цикл
						Если ЗначениеЗаполнено(Строка.СерияНоменклатуры) И МассивСерий.Найти(Строка.СерияНоменклатуры)
							= Неопределено Тогда
						...
						Если Выборка.Следующий() Тогда
							...
							Иначе
								...
								Если ЗаполнятьКоды Тогда
									...
									Серия.КодДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПроектЗадания,
										"ДобывающаяОрганизация.Код");
									Серия.КодГрифа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПоНом.Гриф, "Код");
								КонецЕсли;
...
Процедура РазбитьВсеНаСервере(ЕстьИзменения)
	...
	СтрокиССериями = Объект.Товары.НайтиСтроки(СтруктураОтбора);
	Для Каждого Строка Из СтрокиССериями Цикл
		Если Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Номенклатура, "ВидНоменклатуры.НастройкаИспользованияСерий") = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
			Продолжить;
		КонецЕсли;
		Если Строка.Количество > 1 Тогда
			Серия = Справочники.СерииНоменклатуры.СоздатьЭлемент();
			Серия.КодДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПроектЗадания, "ДобывающаяОрганизация.Код");
			Серия.КодЦентра = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Центр, "Код");
			Серия.КодГрифа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Гриф, "Код");
			Серия.Владелец = Строка.Номенклатура;
			Серия.ПроектЗадания = Объект.ПроектЗадания;
			...
			Для Счетчик = 0 По Строка.Количество - 2 Цикл
				...
				Серия.КодДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПроектЗадания, "ДобывающаяОрганизация.Код");
				Серия.КодЦентра = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Центр, "Код");
				Серия.КодГрифа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоваяСтрока.Гриф, "Код");
