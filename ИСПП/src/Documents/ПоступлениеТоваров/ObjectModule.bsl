...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Строка Из НоменклатураПоСериям Цикл
		Если Строка.Количество > 1 И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.Номенклатура, "ВидНоменклатуры.НастройкаИспользованияСерий") = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	РА_ПроектЗадания = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказПоставщику, "ПроектЗадания");
	
	Если ЗначениеЗаполнено(ЗаказПоставщику) Тогда
		Если ПроектЗадания <> РА_ПроектЗадания Тогда
			ПроектЗадания = РА_ПроектЗадания;
		КонецЕсли;
	Иначе
	...
	Если Не Отказ Тогда
		...
		Для Каждого Строка Из СтрокиССериями Цикл
			...
			Если Не НеИзменятьСерии Тогда
				...
				Серия.КодЦентра = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Центр, "Код");
				Серия.КодДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроектЗадания, "ДобывающаяОрганизация.Код");
...
Процедура ЗаполнитьДокументНаОснованииСпецификацииКДоговору(ДокументОснование)
	...
	ДопустимыеСтатусы.Добавить(Перечисления.СтатусыСпецификацийКДоговорам.ВыполненоЧастично);
	ДопустимыеСтатусы.Добавить(Перечисления.СтатусыСпецификацийКДоговорам.Выполнено);
