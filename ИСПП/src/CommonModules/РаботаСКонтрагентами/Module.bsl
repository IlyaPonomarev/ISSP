...
// Устарела. Следует использовать ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИНН.
...
Функция ИнформацияОПроверкахКонтролирующимиОрганамиПоИНН(Знач ИНН, Плановые, Внеплановые, Завершенные, Незавершенные) Экспорт
	...
	Результат = НоваяИнформацияОПроверкахКонтролирующимиОрганами();
	Результат.ИНН = ИНН;
	
	СписокИНН = Новый Массив;
	СписокИНН.Добавить(ИНН);
	РезультатПоСпискуИНН = ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИНН(
		СписокИНН,
		Плановые,
		Внеплановые,
		Завершенные,
	...
	ЗаполнитьЗначенияСвойств(Результат, РезультатПоСпискуИНН, "ОжиданиеОтвета, ОписаниеОшибки");
	
	Если Результат.ОжиданиеОтвета Или ЗначениеЗаполнено(Результат.ОписаниеОшибки) Тогда
		// Обратная совместимость программного интерфейса.
		Результат.Проверки = НоваяТаблицаПроверкиКонтролирующимиОрганами();
	Иначе
		Результат.Проверки = РезультатПоСпискуИНН.Проверки[ИНН];
		Если ТипЗнч(Результат.Проверки) <> Тип("ТаблицаЗначений") Тогда
			// Обратная совместимость программного интерфейса.
			Результат.Проверки = НоваяТаблицаПроверкиКонтролирующимиОрганами();
		КонецЕсли;
	КонецЕсли;
...
Функция НоваяИнформацияОПроверкахКонтролирующимиОрганами()
	...
	Информация.Вставить("Проверки", Неопределено);
...
Функция АдресКИ(АдресXDTO, Фабрика, АдресныеСокращения)
	...
	ПроверитьДополнительныеАдресныеЭлементы(АдресXDTO.Состав, Фабрика);
	...
	Результат.КонтактнаяИнформация = СериализацияОбъектаXDTO(КИ);
