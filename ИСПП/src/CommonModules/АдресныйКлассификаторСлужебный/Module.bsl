...
// Возвращает актуальные данные адресного объекта или ориентира с точностью до подчиненного (без домов).
//
// Параметры:
...
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Результат.Вставить("Идентификаторы", СтруктураКодовКЛАДРПоУровням());
	...
	Если СтрСравнить(Источник, "Сервис1С") = 0 Тогда
		Результат.КодыАдреса = КодыАдресаСервис(Адрес, Результат.Идентификаторы);
		Результат.ИдентификаторАдресногоОбъекта = Результат.КодыАдреса.Идентификатор;
...
Функция КодыАдресаЗагруженныеДанные(АдресРФ)
	...
	Идентификаторы = СтруктураКодовКЛАДРПоУровням();
	...
	Если ПустаяСтрока(Адрес.Округ) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторОкруг," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыОкруг.Идентификатор КАК ИдентификаторОкруг," + Символы.ПС;
	...
	Если ПустаяСтрока(Адрес.Район) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторРайон," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыРайон.Идентификатор КАК ИдентификаторРайон," + Символы.ПС;
	...
	Если ПустаяСтрока(Адрес.Город) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторГород," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыГород.Идентификатор КАК ИдентификаторГород," + Символы.ПС;
	...
	Если ПустаяСтрока(Адрес.ВнутригРайон) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторВнутригородскойРайон," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыВнутригородскойРайон.Идентификатор КАК ИдентификаторВнутригородскойРайон," + Символы.ПС;
	...
	Если ПустаяСтрока(Адрес.НаселенныйПункт) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторНаселенныйПункт," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыНаселенныйПункт.Идентификатор КАК ИдентификаторНаселенныйПункт," + Символы.ПС;
	...
	Если ПустаяСтрока(Адрес.Улица) Тогда
		...
		|НЕОПРЕДЕЛЕНО КАК ИдентификаторУлица," + Символы.ПС;
	Иначе
		...
		|АдресныеОбъектыУлица.Идентификатор КАК ИдентификаторУлица," + Символы.ПС;
	...
	|	АдресныеОбъектыРегион.Идентификатор КАК ИдентификаторРегион,
	...
	Если РезультатЗапроса.Следующий() Тогда
		...
		УстановитьИдентификатор(Результат.Идентификаторы, "Регион", РезультатЗапроса.ИдентификаторРегион, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "Округ", РезультатЗапроса.ИдентификаторОкруг, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "Район", РезультатЗапроса.ИдентификаторРайон, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "Город", РезультатЗапроса.ИдентификаторГород, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "ВнутригородскойРайон", РезультатЗапроса.ИдентификаторВнутригородскойРайон, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "НаселенныйПункт", РезультатЗапроса.ИдентификаторНаселенныйПункт, ИдентификаторАдресногоОбъекта);
		УстановитьИдентификатор(Результат.Идентификаторы, "Улица", РезультатЗапроса.ИдентификаторУлица, ИдентификаторАдресногоОбъекта);
...
//  Структура - Коды адреса.
//
Функция КодыАдресаСервис(АдресРФ, Идентификаторы)
	...
	Попытка
		ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(РезультатВебСервис, АдресаДляПроверки, Истина);
	Исключение
		СтруктураОписанияОшибкиПоставщика(РезультатВебСервис, ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации( СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , , РезультатВебСервис.ПодробноеПредставлениеОшибки);
	КонецПопытки;
	...
	Если НЕ РезультатВебСервис.Отказ Тогда
		Если РезультатВебСервис.Данные[0].Варианты.Количество() > 0 Тогда
			...
			Если ТипЗнч(Идентификаторы) = Тип("Структура") Тогда
				Идентификаторы = РезультатВебСервис.Данные[0].Идентификаторы;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьИдентификатор(Идентификаторы, ИмяПоля, Значение, Идентификатор)
	Идентификаторы[ИмяПоля] = Значение;
	Если ЗначениеЗаполнено(Значение) Тогда
...
//
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, АдресаДляПроверки, ДобавитьИдентификаторы = Ложь)
	...
	Для Каждого ЭлементПроверки Из РезультатПроверки.Item Цикл
		...
		Если ДобавитьИдентификаторы Тогда
			
			СтруктураКодов = СтруктураКодовКЛАДРПоУровням();
			ЗаполнитьИдентификаторыАдресаПоКлассификаторуСервис1С(Результат, СписокДляПроверки.Item[Индекс].Address, СписокДляПроверки.Item[Индекс].Levels, СтруктураКодов);
			ДанныеДляАдреса.Вставить("Идентификаторы", СтруктураКодов);
		КонецЕсли;
...
// Заполнение результата проверки адресного объекта по данным сервиса.
//
Процедура ЗаполнитьИдентификаторыАдресаПоКлассификаторуСервис1С(Результат, АдресДляПроверки, Уровни, СтруктураКодов)
	...
	КопияАдреса = АдресДляПроверки;
	
	ПроверяемыйАдрес =  СписокДляПроверки.Item.Добавить(Сервис.ФабрикаXDTO.Создать(ТипЭлементаСписка));
	АдресВXML = ОбщегоНазначения.ОбъектXDTOВСтрокуXML(АдресДляПроверки);
	АдресВXML = СтрЗаменить(АдресВXML, "http://www.v8.1c.ru/ssl/contactinfo_ru", "http://www.v8.1c.ru/ssl/contactinfo"); // Локализация
	ПроверяемыйАдрес.Address = ОбщегоНазначения.ОбъектXDTOИзСтрокиXML(АдресВXML, Сервис.ФабрикаXDTO);
	ПроверяемыйАдрес.Levels  = Уровни;
	
	КодЯзыка = ТекущийКодЛокализации();
	
	СоответствиеУровней = Новый Соответствие;
	СоответствиеУровней.Вставить("Улица", "Улица");
	СоответствиеУровней.Вставить("НаселПункт", "НаселенныйПункт");
	СоответствиеУровней.Вставить("ВнутригРайон", "ВнутригородскойРайон");
	СоответствиеУровней.Вставить("Город", "Город");
	СоответствиеУровней.Вставить("СвРайМО", "Район");
	СоответствиеУровней.Вставить("Округ", "Округ");
	СоответствиеУровней.Вставить("СубъектРФ", "Регион");
	
	СписокУровней = Новый Массив;
	СписокУровней.Добавить("Улица");
	СписокУровней.Добавить("НаселПункт");
	СписокУровней.Добавить("ВнутригРайон");
	СписокУровней.Добавить("Город");
	СписокУровней.Добавить("СвРайМО");
	СписокУровней.Добавить("Округ");
	СписокУровней.Добавить("СубъектРФ");
	
	ЗаполненныеУровни = Новый Массив;
	
	Для каждого ИмяУровня Из СписокУровней Цикл
		Значение = АдресДляПроверки[ИмяУровня];
		Если ТипЗнч(Значение) = Тип("ОбъектXDTO") И ИмяУровня= "СвРайМО" Тогда
			Значение = Значение.Район;
		КонецЕсли;
		Если ЗначениеЗаполнено(Значение) Тогда
			ЗаполненныеУровни.Добавить(ИмяУровня);
		КонецЕсли;
	КонецЦикла;
	
	Для каждого ИмяУровня Из ЗаполненныеУровни Цикл
	
		РезультатПроверки = Сервис.Analyze(СписокДляПроверки, КодЯзыка, Ложь, Метаданные.Имя);
		
		Если РезультатПроверки.Item.Количество() > 0 И РезультатПроверки.Item[0].Variant.Количество() Тогда
			СтруктураКодов[СоответствиеУровней[ИмяУровня]] = РезультатПроверки.Item[0].Variant[0].ID;
		КонецЕсли;
		
		Если ИмяУровня <> "СвРайМО" Тогда
			СписокДляПроверки.Item[0].Address[ИмяУровня] = "";
		Иначе
			СписокДляПроверки.Item[0].Address[ИмяУровня].Район = "";
		КонецЕсли;
