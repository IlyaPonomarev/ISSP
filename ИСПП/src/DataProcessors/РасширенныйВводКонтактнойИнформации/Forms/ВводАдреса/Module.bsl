...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Если ЗначениеЗаполнено(ОсновнаяСтрана) Тогда
			СтранаПоУмолчанию = ОсновнаяСтрана;
		Иначе
			СтранаПоУмолчанию = РаботаСАдресами.ОсновнаяСтрана();
		КонецЕсли;
		Страна    = СтранаПоУмолчанию;
		КодСтраны = СтранаПоУмолчанию.Код;
	КонецЕсли;
	
	ОтобразитьДополнительныеЗданияИПомещения();
	
	Если ТолькоНациональныйАдрес И Не МеждународныйФорматАдреса Тогда
	...
	ИначеЕсли Страна = ОсновнаяСтрана Тогда
...
Процедура ОбработатьКонтактнуюИнформациюСИсторией(Результат)
	...
	Если ДатаБылаИзменена Тогда
		...
		Если СтрСравнить(Результат.Представление, ПредставлениеРедактируемогоАдреса) <> 0 
			Или СтранаОтличается(СтрокиСАдресом[0].ЗначенияПолей, Результат.КонтактнаяИнформация) Тогда
	...
	Иначе
		Если СтрСравнить(Результат.Представление, СтрокаДействующегоАдреса.Представление) <> 0
			Или СтрСравнить(Результат.Комментарий, СтрокаДействующегоАдреса.Комментарий) <> 0
			Или СтранаОтличается(СтрокаДействующегоАдреса.ЗначенияПолей, Результат.КонтактнаяИнформация) Тогда
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если (Не ЭтоНациональныйАдрес И ЭтоСтранаУчастникЕАЭС(Страна)) Или МеждународныйФорматАдреса Тогда
		// ЕАЭС
		Контекст = КонтекстФормыКлиент();
...
Функция СтранаОтличается(ЗначениеПолейДо, ЗначениеПолейПосле)
	
	СтранаДо    = УправлениеКонтактнойИнформацией.СтранаАдресаКонтактнойИнформации(ЗначениеПолейДо);
	СтранаПосле = УправлениеКонтактнойИнформацией.СтранаАдресаКонтактнойИнформации(ЗначениеПолейПосле);
	
	Возврат СтрСравнить(СтранаДо.Наименование, СтранаПосле.Наименование) <> 0;
...
Процедура СообщениеОНеобходимостиОбновленияКлассификатора(Идентификатор = Неопределено)
	...
	Если Страна = ОсновнаяСтрана Тогда
		ВидимостьНеобходимоОбновление = НЕ ЭтоВебСервис И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор");
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
...
Процедура ОбработкаИзмененияСтраныКлиент()
	...
	Если МеждународныйФорматАдреса Тогда
		Элементы.ТипАдреса.ТекущаяСтраница = Элементы.МеждународныйАдрес;
	ИначеЕсли ЭтоНациональныйАдрес Тогда
		Элементы.ТипАдреса.ТекущаяСтраница = Элементы.НациональныйАдрес;
	Иначе
...
Процедура ОпределениеРежимаВводаАдреса()
	...
	Иначе
		...
		Элементы.ВвестиАдресВСвободнойФормеВсеДействия.Доступность = Истина;
		Если РазрешитьВводАдресаВСвободнойФорме Тогда
			Элементы.СтруктураМеждународныеАдреса.Видимость = Ложь;
			Элементы.ГруппаИндекса.Видимость                = Ложь;
		Иначе
			Элементы.СтруктураМеждународныеАдреса.Видимость = Истина;
			Элементы.ГруппаИндекса.Видимость                = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	СообщениеОНеобходимостиОбновленияКлассификатора(Неопределено);
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если МеждународныйФорматАдреса Тогда
		Элемент = Элементы.ПредставлениеМеждународногоАдреса;
	Иначе
		Если Страна = ОсновнаяСтрана Тогда
	...
	Если РедактироватьПредставлениеАдреса Тогда
		...
		Если МеждународныйФорматАдреса Тогда
			ТекущийЭлемент = Элементы.ПредставлениеМеждународногоАдреса;
		ИначеЕсли Страна <> ОсновнаяСтрана Тогда
	...
	Иначе 
		...
		Если МеждународныйФорматАдреса Тогда
			ТекущийЭлемент = Элементы.НаселенныйПунктМеждународныеАдреса;
		ИначеЕсли Страна = ОсновнаяСтрана Тогда
...
Процедура СостояниеГруппыВвода(Группа, Режим)
	...
	Для Каждого Элемент Из Группа.ПодчиненныеЭлементы Цикл
		...
		Если ТипЭлемента = Тип("ГруппаФормы") Тогда
			Если Элемент <> Элементы.ИностранныйАдрес И Элемент <> Элементы.МеждународныйАдрес Тогда
