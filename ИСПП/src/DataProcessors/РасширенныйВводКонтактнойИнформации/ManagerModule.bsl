...
Процедура ЗаполнитьКодыАдреса(Результат, Адрес)
	...
	Идентификаторы = Новый Структура("Регион, Округ, Город, ВнутригородскойРайон, НаселенныйПункт, Улица");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Результат.Идентификаторы = АдресныеКоды.Идентификаторы;
		Результат.ИдентификаторАдресногоОбъекта = АдресныеКоды.ИдентификаторАдресногоОбъекта;
	КонецЕсли;
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если ТипЗнч(АдресРФ) <> Тип("ОбъектXDTO") Или АдресРФ.Тип() <> ФабрикаXDTO.Тип(РаботаСАдресами.ПространствоИмен(), "АдресРФ") Тогда
		...
		Если ФлагиПроверки.МеждународныйФорматАдреса Тогда
			СтрокаПроверки = СтрЗаменить(АдресРФ, XDTOАдрес.Страна, "");
			Если Не СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(СтрокаПроверки, Ложь, "1234567890") Тогда
