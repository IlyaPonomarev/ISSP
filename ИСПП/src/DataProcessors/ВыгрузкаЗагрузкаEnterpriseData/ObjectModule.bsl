...
Функция ВыгрузитьДанныеВXML()
	...
	ТекстВыгрузки = ТекстВыгрузки + Символы.ПС + "</Body>" + Символы.ПС + "</Message>";
...
Процедура ЗагрузкаСообщения(СтруктураПараметров, АдресРезультата) Экспорт
	...
	Если РезультатЗагрузки.ЕстьОшибки Тогда
		ТекстСообщения = НСтр("ru='В ходе выполнения операции возникли ошибки'") + ": " + Символы.ПС + РезультатЗагрузки.ТекстОшибки;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
