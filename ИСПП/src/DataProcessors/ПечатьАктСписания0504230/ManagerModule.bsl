...
Функция ДобавитьПечатнуюФорму(ПечатныеФормы) Экспорт
	...
	ПечатнаяФорма = УправлениеПечатьюПоддержкаПроектов.ДобавитьПечатнуюФорму(ПечатныеФормы, "АктСписания0504230", МенеджерПечати);
...
//
Функция ПечатьАктСписания0504230(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ПараметрыПечати.Вставить("ДанныеСписания");
...
Процедура СформироватьТабличныйДокумент(ТабличныйДокумент, Макет, ДанныеДляПечати, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ФормированиеПечатныхФормПоддержкаПроектов.ВывестиОбластьПоИмени(ТабличныйДокумент, Макет, "ШапкаТаблицы");
		...
		ОбластиЗаключениеКомиссии = Новый Массив;
		ОбластиЗаключениеКомиссии.Добавить(Макет.ПолучитьОбласть("ЗаключениеНачало"));
		ОбластиЗаключениеКомиссии.Добавить(Макет.ПолучитьОбласть("ЗаключениеОкончание"));
		ОбластиЗаключениеКомиссии.Добавить(Макет.ПолучитьОбласть("ЗаключениеОкончание"));
		
		ПроверитьВыводПодвала(ТабличныйДокумент, Макет, МассивВыводимыхОбластей, ОбластиЗаключениеКомиссии, ОбластиКомиссии);
		
		// Вывод областей Заключение
		Для Каждого ОбластьЗаключениеКомиссии Из ОбластиЗаключениеКомиссии Цикл
			ТабличныйДокумент.Вывести(ОбластьЗаключениеКомиссии);
		КонецЦикла;
		...
		ФормированиеПечатныхФормПоддержкаПроектов.ВывестиОбластьПоИмени(ТабличныйДокумент, Макет, "Дата", ДанныеШапки);
...
Функция СформироватьОбластиКомиссии(Макет, Шапка, ВыборкаПоКомиссиямДокументов, ДанныеШапки)
	...
	Для НомерЧленаКомиссии = 1 По Макс(КоличествоЧленовКомиссии, ПустыеСтроки) Цикл
		...
		ФормированиеПечатныхФормПоддержкаПроектов.ДобавитьОбластьВМассивПоОписанию(Макет, ИмяОбласти, ОбластиКомиссии, Параметры);
...
Функция ПолучитьПричинуСписания(Данные)
	...
	ДанныеПричиныСписания = Новый Структура("СтатьяРасходов, АналитикаРасходов, ПричинаСписания");
	ЗаполнитьЗначенияСвойств(ДанныеПричиныСписания, Данные);
	
	Если ЗначениеЗаполнено(ДанныеПричиныСписания.ПричинаСписания) Тогда
		ПричинаСписания = ДанныеПричиныСписания.ПричинаСписания;
	ИначеЕсли ЗначениеЗаполнено(ДанныеПричиныСписания.СтатьяРасходов) Тогда
...
Процедура ПроверитьВыводПодвала(ТабличныйДокумент, Макет, МассивВыводимыхОбластей, ОбластиЗаключениеКомиссии, ОбластиКомиссии)
	
	МассивВыводимыхОбластей.Очистить();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивВыводимыхОбластей, ОбластиЗаключениеКомиссии);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивВыводимыхОбластей, ОбластиКомиссии);
	МассивВыводимыхОбластей.Добавить(Макет.ПолучитьОбласть("Дата"));
...
Функция МетаданныеМакета()
	...
	Возврат Метаданные.Обработки.ПечатьАктСписания0504230.Макеты.ПФ_MXL_АктСписания0504230;
