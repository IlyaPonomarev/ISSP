...
Функция ДобавитьПечатнуюФорму(ПечатныеФормы) Экспорт
	...
	ПечатнаяФорма = УправлениеПечатьюПоддержкаПроектов.ДобавитьПечатнуюФорму(ПечатныеФормы, "НакладнаяНаОтпускМатериалов0504205", МенеджерПечати);
		...
		Если ВыборкаПоДокументам.НайтиСледующий(ПараметрыПоиска) Тогда
			...
			Пока ВыборкаСтрокТовары.Следующий() Цикл
				...
				ТоварНаименование = ОбщегоНазначенияПоддержкаПроектов.ПолучитьПредставлениеНоменклатурыДляПечати(
					...
					ВыборкаСтрокТовары.СерияНоменклатуры.ЗаводскойНомер);
...
Функция ПолучитьДанныеШапкиДокумента(Шапка)
	...
	// Данные заголовка
	Если Не ЗначениеЗаполнено(Шапка.Отправитель.ДополнительноеЮрЛицо) Тогда
		СведенияОбОтправителе = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Шапка.Отправитель, Шапка.ДатаДокумента);
	Иначе
		СведенияОбОтправителе = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Шапка.Отправитель.ДополнительноеЮрЛицо, Шапка.ДатаДокумента);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Шапка.Ссылка.НомерПриказа) Тогда
		ПолучательФИО =  ФизическиеЛицаКлиентСервер.ФамилияИнициалы(СведенияОПолучателе.НаименованиеДляПечатныхФорм);
		Параметры.Вставить("ПолучательФИО"       , ПолучательФИО);
		Параметры.Вставить("Приказ", "Приказ Федеральной службы охраны Российской Федерации" + " № " + Шапка.Ссылка.НомерПриказа + " от " + Формат(Шапка.Ссылка.ДатаПриказа, "ДЛФ=Д"));
		Параметры.Вставить("ТекстОбОружии", "С выпиской из Федерального закона Российской Федерации ""Об оружии"" ознакомлен:  _______________  " + ПолучательФИО); 	
	КонецЕсли;
