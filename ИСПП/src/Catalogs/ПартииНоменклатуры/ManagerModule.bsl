...
Функция ПолучитьНаименованиеКлючаАналитики(ПараметрыАналитики) Экспорт
	...
	Если ЗначениеЗаполнено(ОжидаемыеПоля.ДокументОприходования) Тогда
		ПредставлениеДокумента = ОбщегоНазначенияПоддержкаПроектов.ПредставлениеОбъекта(ОжидаемыеПоля.ДокументОприходования.Метаданные());
...
Функция ОбластиПоиска(ПолнотекстовыйПоиск) Экспорт
	...
	ОбластиПоиска.Добавить(Метаданные.Справочники.ПартииНоменклатуры);
...
Функция ТекстЗапросаПоискаПоОбласти(ОбластьПоиска) Экспорт
	...
	Если ОбластьПоиска = Метаданные.Справочники.ПартииНоменклатуры Тогда
		...
		|	Партии.Ссылка КАК Значение,
		|	""ПартииНоменклатуры"" КАК Метаданные
		|ИЗ
		|	Справочник.ПартииНоменклатуры КАК Партии
		|ГДЕ
		|	Партии.Наименование %ВидСравнения% &СтрокаПоиска %Спецсимвол%
		|";
	КонецЕсли;
...
Функция ОбработатьНайденныеДанные(НайденныеДанные) Экспорт
	...
	РезультатПоиска.Вставить("Партии", Новый Массив);
	
	Для Каждого Элемент Из НайденныеДанные Цикл
		Если ТипЗнч(Элемент.Метаданные) = Тип("ОбъектМетаданных") Тогда
			
			Если Элемент.Метаданные = Метаданные.Справочники.ПартииНоменклатуры Тогда
				РезультатПоиска.Партии.Добавить(Элемент.Значение);
			Иначе
		...
		Иначе
			...
			Если Элемент.Метаданные = "ПартииНоменклатуры" Тогда
				РезультатПоиска.Партии.Добавить(Элемент.Значение);
			Иначе
				ВызватьИсключение НСтр("ru = 'Неизвестная ошибка'");
			КонецЕсли;
