...
//
Функция ДобавитьЕдиницыИзмеренияПоКоду(Знач Коды, РежимОбновления = Ложь) Экспорт
...
Функция ТекстЗапросаКоличествоВМерныхЕдиницах(ТипЕдиницы, ИсточникНоменклатуры = Неопределено) Экспорт
	...
	|		ВЫРАЗИТЬ(
	|			ВЫРАЗИТЬ(ТаблицаТовары.Номенклатура.%Мера%Числитель / ТаблицаТовары.Номенклатура.%Мера%Знаменатель КАК ЧИСЛО(15, 7))
	|			* ТаблицаТовары.Номенклатура.%Мера%ЕдиницаИзмерения.Коэффициент КАК ЧИСЛО(15, 7))
	|	ИНАЧЕ
	|		0
	|КОНЕЦ
	|";
